...
PassNames getBufferizationStage(bool asyncQNodes = false)
{
    const std::string bufferizationOptions =
        ...
        "}";
    PassNames ret;
    ret.reserve(pipelineList[3].passNames.size());
    for (const auto &passName : pipelineList[3].passNames) {
        if (passName == "one-shot-bufferize") {
            ret.push_back(passName + bufferizationOptions);
        }
        else {
            ret.push_back(passName);
        }
    }
    return ret;
}
